...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        getMsgSelected_edit: function (n) {
            var self = this, strFiles = self.filePlural;
            return self.msgSelected.replace('{n}', n).replace('{files}', strFiles);
        },
            ...
            self.$preview.find('.kv-file-remove').each(function () {
                ...
                handler($el, 'click', function () {
                    ...
                    $frame.fadeOut('slow', function () {
                        ...
                        //valida_modal_edicion
                        if (($("div#ind_modal_edit_insert").html()) === "0") { //verifica de cual modal se abre, de la insertar o editar
                            var numImgPrevia_remove = $('#modal_noticia_editar div.image_previa_bd').size();
                            var len = filestack.length + numImgPrevia_remove;
                        ...
                        if (len === 0 && chk === 0 && !hasThumb) {
                            ...
                            if (n > 1) {
                                cap = self.getMsgSelected(n);
                            } else {
                                //valida_modal_edicion
                                if (($("div#ind_modal_edit_insert").html()) === "0") {
                                    cap = self.getMsgSelected_edit(n);
                                }
                                else {
                                    if (filestack[0]) {
                                        cap = self.getFileNames()[0];
                                    } else {
                                        cap = '';
                                    }
                                }
        ...
        updateFileDetails: function (numFiles) {
            ...
            if (($("div#ind_modal_edit_insert").html()) === "0") { //verifica de cual modal se abre, de la insertar o editar
                var numImgPrevia = 0;
                //aqui limpio el nombre del archivo adjunto que viene cargado de la bd, para poder concatenar con los otros nuevos archivos que quiero incorporar
                $("div").remove('.file_titulo_edit');
                numImgPrevia = $('#modal_noticia_editar div.image_previa_bd').size();
                var self = this, $el = self.$element, fileStack = self.getFileStack(),
                    name = $el.val() || (fileStack.length && fileStack[0].name) || '', label = self.slug(name);
                if (self.isUploadable) {
                    var n = numImgPrevia + fileStack.length;
                }
                else {
                    var n = numImgPrevia + numFiles;
                }
                var nFiles = previewCache.count(self.id) + n,
